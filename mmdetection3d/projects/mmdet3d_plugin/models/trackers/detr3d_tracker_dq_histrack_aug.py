...
class DETR3DTrackerDQ(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.pe = nn.Sequential(nn.Linear(3, tracker_cfg.embed_dims), nn.LayerNorm(tracker_cfg.embed_dims))
        self.his_track_posnet = nn.Sequential(nn.Linear(3, tracker_cfg.embed_dims), nn.LayerNorm(tracker_cfg.embed_dims))
        ...
        self.img_track_cross_attn = nn.TransformerDecoder(self.img_track_cross_attn_decoder_layer, num_layers=6)
    ...
    def forward_detector(self, img, img_metas, detector, tracklist=[], infra_label=False):
        ...
        if len(tracklist) > 0:
            ...
            img_feats[0] = img_feats[0] + 0.1*img_pe
            ...
            #ego2lidar = torch.inverse(torch.tensor(img_metas[0]['lidar2ego'][0][6:,:,:], dtype=torch.float32).to(device=his_pos.device))
            ...
            #pts_ego_homo = torch.cat([his_pos_exp, torch.ones_like(his_pos_exp[..., :1])], dim=-1)
            #pts_lidar = torch.matmul(pts_ego_homo, ego2lidar.T)
            
            #his_pos_global = torch.matmul(his_pos_exp, l2g_r.transpose(1, 2)) + l2g_t.unsqueeze(1)
            pe = self.his_track_posnet(his_pos_global)
            his_query_exp = his_query_exp + 0.01*pe
            ...
            #pts_lidar = torch.cat([his_pos_exp, torch.ones_like(his_pos_exp[..., :1])], dim=-1)
            ...
            #shape = pts_img[...,2].shape
            #z = torch.empty(shape, dtype=torch.float32, device='cuda').uniform_(10.0, 30.0)
            
            u_pts = pts_img[..., 0] / (pts_img[..., 2] + 1e-6)  # [B, Num]
            v_pts = pts_img[..., 1] / (pts_img[..., 2] + 1e-6)
            img_h, img_w = img_metas[0]['img_shape'][0][:2]

            #feat = img[0, 2].detach().cpu() 
            #post_img = feat.mean(dim=0).cpu()  # [H, W]
            #plt.scatter(u_pts[2].detach().cpu(), v_pts[2].detach().cpu(), c='red', s=10)
            ...
            sampled_feats_pe_emb = nn.Embedding(max_len, C).to(sampled_feats.device)
            pe_norm_net = nn.LayerNorm(C).to(sampled_feats.device)
            sample_feats_position = torch.arange(max_len, device=sampled_feats.device).unsqueeze(0).expand(B*T, -1)  # [B*T, max_len]
            sampled_feats_pe = sampled_feats_pe_emb(sample_feats_position)
            sampled_feats_pe_norm = pe_norm_net(sampled_feats_pe)
            sampled_feats = sampled_feats + sampled_feats_pe_norm
            
            
            attn_out = self.img_track_cross_attn(tgt=sampled_feats, memory=his_query_exp)
            attn_out = 10*attn_out
            ...
            img_feats = [img_feats_flat_aug.permute(0, 2, 1).reshape(B, T, C, H, W)]
        
            #feat = img_feats[0][0, 2].detach().cpu() 
            #post_img = feat.mean(dim=0).cpu()  # [H, W]
            #plt.imshow(post_img, cmap='gray')
            #plt.axis('off')
            #plt.show()
    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            for i in unmatched_i:
                new_detections[i+current_size] = (detector_i.output_embedding[i], detector_i.pred_boxes[i], detector_i.pred_logits[i], detector_i.scores[i], detector_i.global_pts[i], detector_i.pos_velo[i])
