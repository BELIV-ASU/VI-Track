...
def moment_matching_loss(fused_embedding_i, fused_embedding_v):
    # mean alignment
    mean_i = fused_embedding_i.mean(0)
    mean_v = fused_embedding_v.mean(0)
    mean_loss = torch.norm(mean_i - mean_v, p=2)

    # covariance alignment
    cov_i = torch.cov(fused_embedding_i.T)
    cov_v = torch.cov(fused_embedding_v.T)
    cov_loss = torch.norm(cov_i - cov_v, p='fro')
...
class DETR3DTrackerDQ(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        # Modification:
        ## Query fusion
        self.fusion_mlp = nn.Sequential(nn.Linear(568, 512),
                                        nn.LayerNorm(512),
                                        nn.ReLU(),
                                        nn.Linear(512, 284),
                                        nn.Sigmoid())
        ...
        self.mlp = nn.Sequential(nn.Linear(24, tracker_cfg.embed_dims),
                                   nn.LayerNorm(tracker_cfg.embed_dims),
                                   nn.ReLU(inplace=False),
                                   nn.Linear(tracker_cfg.embed_dims,tracker_cfg.embed_dims),
                                   nn.LayerNorm(tracker_cfg.embed_dims),
                                   nn.ReLU(inplace=False),
                                   nn.Linear(tracker_cfg.embed_dims,tracker_cfg.embed_dims))
        self.emb_proj = nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims)
        ...
        #---------- Modification for IV conference-------------------
        ## Vehicle model detection augmentation 
        #self.value_net = nn.Linear(tracker_cfg.embed_dims, 1)
        #self.fusion_proj = nn.Linear(2*tracker_cfg.embed_dims, tracker_cfg.embed_dims)

        ## Vehicle model tracking augmentation
        #self.track_error_ca_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, activation='relu', batch_first=True)
        #self.track_error_ca = nn.TransformerDecoder(self.track_error_ca_decoder_layer, num_layers=6)
        #self.time_net = nn.Linear(1, tracker_cfg.embed_dims)
        #self.pred_error_net = nn.Linear(3*tracker_cfg.embed_dims, 1)
        
        ## Fusion (Feature Synchronization)
        #M1 & M2
        self.kv = nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims)
        self.img_track_cross_attn_decoder_layer_v = nn.TransformerDecoderLayer(d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, activation='relu', batch_first=True)
        self.img_track_cross_attn_v = nn.TransformerDecoder(self.img_track_cross_attn_decoder_layer_v, num_layers=4)
        self.img_track_cross_attn_decoder_layer_i = nn.TransformerDecoderLayer(d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, activation='relu', batch_first=True)
        self.img_track_cross_attn_i = nn.TransformerDecoder(self.img_track_cross_attn_decoder_layer_i, num_layers=4)

        # Proposed M
        self.img_track_cross_attn_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, activation='relu', batch_first=True)
        self.img_track_cross_attn = nn.TransformerDecoder(self.img_track_cross_attn_decoder_layer, num_layers=4)
        #self.img_track_self_attn_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, activation='relu', batch_first=True)
        #self.img_track_self_attn = nn.TransformerDecoder(self.img_track_self_attn_decoder_layer, num_layers=4)
        self.emb_proj_i = nn.Sequential(nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims), nn.LayerNorm(tracker_cfg.embed_dims), nn.ReLU(), nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims))
        self.emb_proj_v = nn.Sequential(nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims), nn.LayerNorm(tracker_cfg.embed_dims), nn.ReLU(), nn.Linear(tracker_cfg.embed_dims, tracker_cfg.embed_dims))

        # discriminator method
        #self.Dis = nn.Sequential(nn.Linear(tracker_cfg.embed_dims,128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU(),  nn.Linear(128,2))
        self.Dis = nn.Sequential(nn.Linear(tracker_cfg.embed_dims, 256),
                                 nn.LayerNorm(256),
                                 nn.LeakyReLU(0.2),
                                 nn.Dropout(0.3),
        ...
        self.sa_v = nn.Sequential(nn.Linear(2, tracker_cfg.embed_dims),nn.LayerNorm(tracker_cfg.embed_dims))
        self.sa_i = nn.Sequential(nn.Linear(2, tracker_cfg.embed_dims),nn.LayerNorm(tracker_cfg.embed_dims))
    ...
    def query_asso(self, detector, tracker, obj_embedding, track_embedding, time_delta):
        ...
        # ----------------IV modification: Vehicle model assisted tracking ----------------#
    ...
    def tracklet_asso_aug(self, tracker, detector, match_info):
        ...
        if aug_cfg['fp_ratio'] > 0:
            ...
            if len(fp_det) > 0:
                ...
                tracker.global_pts[track_idx] = fp_det.global_pts
    ...
    def tracklet_update_train(self, tracker, detector, track_embedding, match_info):
        ...
        for _idx, obj_id in enumerate(obj_ids):
            ...
            tracker.global_pts[track_idx] = detector.global_pts[pos_inds[_idx]]
    ...
    def tracklet_update_test(self, tracker, detector, track_embedding_nb, time_delta, 
        ...
        if len(match_pairs) > 0:
            ...
            tmp_tracker.global_pts[match_tracks] = tmp_detector.global_pts[match_dets]
    ...
    def tracklet_asso_train(self, tracker, detector, match_info, tracklist, time_delta=None):
        pos_inds = match_info['pos_inds']
        # embedding interaction
        track_embedding, obj_embedding = self.embedding_interaction(detector[pos_inds],tracklist)
    ...
    def tracklet_asso_test(self, tracker, detector, tracklist, time_delta=None, img_metas=None):
        ...
        # embedding interaction
        track_embedding_nb, obj_embedding = self.embedding_interaction(detector[valid_det], tracklist)
        ...
        if valid_track.sum() == 0:
            ...
            tracker.global_pts[:det_num] = detector.global_pts[valid_det]
        ...
        if len(unmatched_dets) > 0:
            ...
            tracker.global_pts[empty_track] = detector.global_pts[valid_det][unmatched_dets]
    ...
    def sort_and_filter(self, detector, img_metas):
        ...
        l2e_mat = torch.from_numpy(img_metas[0]['lidar2ego'][0][0]).to(pred_cls.device)
    ...
    def trans2real_world(self, detector, img_metas):
        ...
        detector.global_pts = global_pts
    ...
    def forward_train(self,
        ...
        for batch_idx in range(batch_num):
            ...
            for frame_idx in range(num_frame):
                ...
                delay_idx = int(img_metas_single_v[0]['filename'][0].split("_")[-1].split(".")[0])
                if delay_idx <=30:
                    delay = 0.2*2
                elif 30 < delay_idx <= 60:
                    delay = 0.2*4
                elif 60 < delay_idx <= 90:
                    delay = 0.2*3
                else:
                    delay = 0

                pos_velo_i = detector_i.pos_velo.clone()     # full copy
                pred_boxes_i = detector_i.pred_boxes.clone() # full copy
                vx, vy = detector_i.pos_velo[:, 2].clone(), detector_i.pos_velo[:, 3].clone()
                pos = detector_i.pos_velo[:, :2].clone()
                psi = torch.atan2(detector_i.pred_boxes[:,6], detector_i.pred_boxes[:,7] + 1e-8)

                denom = detector_i.pred_boxes[:, 3].clamp(min=0.5, max=20.0)
                omega = (vx * torch.sin(psi) - vy * torch.cos(psi)) / denom

                # If ω too large, treat as straight (ω = 0)
                omega_max = 5.0
                omega = torch.where(torch.abs(omega) > omega_max,
                                    torch.zeros_like(omega),
                                    omega)

                half_omega_t = 0.5 * omega * delay
                sinc = torch.sin(half_omega_t) / (half_omega_t + 1e-6)
                sinc = torch.where(torch.abs(half_omega_t) < 1e-4,
                                   torch.ones_like(half_omega_t),
                                   sinc)
                v = torch.sqrt(vx**2 + vy**2)
                pos_x = pos[:,0] + v * delay * sinc * torch.cos(psi + half_omega_t)
                pos_y = pos[:,1] + v * delay * sinc * torch.sin(psi + half_omega_t)
                psi_t = psi + omega * delay
                
                
                new_pos_velo = detector_i.pos_velo.clone()
                new_boxes = detector_i.pred_boxes.clone()
                new_pos_velo[:, 0] = pos[:,0] + v * delay * sinc * torch.cos(psi + half_omega_t)
                new_pos_velo[:, 1] = pos[:,1] + v * delay * sinc * torch.sin(psi + half_omega_t)
                #psi_t = psi + omega * delay
                new_boxes[:,6] = torch.sin(psi_t)
                new_boxes[:,7] = torch.cos(psi_t)
                detector_i.pos_velo = new_pos_velo
                detector_i.pred_boxes = new_boxes
                
                spatial_embedding_v = self.sa_v(detector_v.pos_velo[:,:2].clone())
                spatial_embedding_i = self.sa_i(detector_i.pos_velo[:,:2].clone())
                detector_i.output_embedding = detector_i.output_embedding.clone() + spatial_embedding_i
                detector_v.output_embedding = detector_v.output_embedding.clone() + spatial_embedding_v

                # ------------- Fusion -------------------------------#
                # Basic fusion
                #fused_embedding_v = detector_v.output_embedding
                #fused_embedding_i = detector_i.output_embedding


                # Method 1: Concat K,V
                #kv_i = self.kv(detector_i.output_embedding).unsqueeze(0)
                #kv_v = self.kv(detector_v.output_embedding).unsqueeze(0)
                #kv = torch.cat((kv_i, kv_v), dim=1)
                #tgt_v = detector_v.output_embedding.unsqueeze(0)   
                #tgt_i = detector_i.output_embedding.unsqueeze(0)
                
                #fused_embedding_v = self.img_track_cross_attn_v(tgt=tgt_v, memory=kv).squeeze(0)
                #fused_embedding_i = self.img_track_cross_attn_i(tgt=tgt_i, memory=kv).squeeze(0)
                ...
                #tgt_v = detector_v.output_embedding.unsqueeze(0)   
                #tgt_i = detector_i.output_embedding.unsqueeze(0)
                #fused_embedding_v = self.img_track_cross_attn_v(tgt=tgt_v, memory=kv_i).squeeze(0)
                #fused_embedding_i = self.img_track_cross_attn_i(tgt=tgt_i, memory=kv_v).squeeze(0)

                # Proposed Method
                fused_embedding_i = self.img_track_cross_attn(tgt=detector_i.output_embedding.unsqueeze(0), memory=detector_v.output_embedding.unsqueeze(0)).squeeze(0)
                #fused_embedding_i = self.img_track_self_attn(tgt=fused_embedding_i, memory=fused_embedding_i).squeeze(0)
                #fused_embedding_v = detector_v.output_embedding

                #fused_embedding_i = detector_i.output_embedding
                fused_embedding_v = detector_v.output_embedding
                fused_embedding_i = self.emb_proj_i(fused_embedding_i)
                fused_embedding_v = self.emb_proj_v(fused_embedding_v)
                loss_mmd = moment_matching_loss(fused_embedding_i, fused_embedding_v)

                # Discriminator method
                pred_i = self.Dis(grad_reverse(fused_embedding_i))
                pred_v = self.Dis(grad_reverse(fused_embedding_v))
                label_i = torch.zeros(len(pred_i), dtype=torch.long, device=fused_embedding_i.device)
                label_v = torch.ones(len(pred_v), dtype=torch.long, device=fused_embedding_v.device)
                loss_align = (F.cross_entropy(pred_i, label_i) + F.cross_entropy(pred_v, label_v)) / 2
                
                # -------------- Fusion End --------------------------#
                # infra -> vehicle relative pose transform
                device = detector_i.global_pts.device
                i2v_points = (detector_i.global_pts - img_metas_single_v[0]['l2g_t'][0].to(device)) @ img_metas_single_v[0]['l2g_r_mat'][0].T.to(device)
                detector_i.pred_boxes[:,:2], detector_i.pred_boxes[:,4:5] = i2v_points[:,:2], i2v_points[:,2].unsqueeze(1)

                fused_embedding_i = F.normalize(fused_embedding_i, dim=-1)
                fused_embedding_v = F.normalize(fused_embedding_v, dim=-1)
                sim_matrix = F.cosine_similarity(fused_embedding_v.unsqueeze(1), fused_embedding_i.unsqueeze(0), dim=-1)

                
                #pseudo_labels_i = sim_matrix.argmax(dim=1)
                #pseudo_labels_v = sim_matrix.argmax(dim=0)
                #loss_i2v = F.cross_entropy(sim_matrix, pseudo_labels_i)
                #loss_v2i = F.cross_entropy(sim_matrix.T, pseudo_labels_v)
                #loss_align = 0.5 * (loss_i2v + loss_v2i)
                
                loss_align_dict = dict()
                loss_align_dict[f'loss_align_batch{batch_idx}_frame{frame_idx}'] = loss_align
                loss_align_dict[f'loss_mmd_batch{batch_idx}_frame{frame_idx}'] = loss_mmd
                loss_dict_single.update(loss_align_dict)
                ...
                diff = detector_v.pos_velo[matched_v, :2] - detector_i.pos_velo[matched_i, :2]
                dist = torch.norm(diff, dim=-1)
                valid_mask = dist <= 1
                matched_v = matched_v[valid_mask]
                matched_i = matched_i[valid_mask]
                ...
                for idx_v, idx_i in zip(matched_v.tolist(), matched_i.tolist()):
                    ...
                    geo_v = torch.cat((detector_v.output_embedding[idx_v],detector_v.pred_boxes[idx_v],detector_v.pred_logits[idx_v],detector_v.scores[idx_v].unsqueeze(0), detector_v.global_pts[idx_v], detector_v.pos_velo[idx_v]),dim=0)
                    geo_i = torch.cat((detector_i.output_embedding[idx_i],detector_i.pred_boxes[idx_i],detector_i.pred_logits[idx_i],detector_i.scores[idx_i].unsqueeze(0), detector_i.global_pts[idx_i], detector_i.pos_velo[idx_i]),dim=0)
                    
                    total_score = detector_v.scores[idx_v] + detector_i.scores[idx_i] + 1e-6  # avoid divide-by-zero
                    weight_v = detector_v.scores[idx_v] / total_score
                    weight_i = detector_i.scores[idx_i] / total_score
                    geo_fusion = weight_v * geo_v + weight_i * geo_i
                    new_detections[idx_v] = (geo_fusion[:256], geo_fusion[256:266], geo_fusion[266:276], geo_fusion[276], geo_fusion[277:280], geo_fusion[280:284])
                    
                
                    merged_v.append(idx_v)
                    merged_i.append(idx_i)
                ...
                for i in unmatched_v:
                    new_detections[i] = (detector_v.output_embedding[i], detector_v.pred_boxes[i], detector_v.pred_logits[i], detector_v.scores[i], detector_v.global_pts[i], detector_v.pos_velo[i])
                ...
                for i in unmatched_i:
                    ...
                    new_detections[i+new_det_size] = (detector_i.output_embedding[i], detector_i.pred_boxes[i], detector_i.pred_logits[i], detector_i.scores[i], detector_i.global_pts[i], detector_i.pos_velo[i])
                ...
                default_global_pts = torch.zeros(D_global_pts).cuda(device)
                default_pos_velo = torch.zeros(D_pos_velo).cuda(device)
                ...
                for i in range(N):
                    if i in new_detections:
                        emb, box, logit, score, global_pts, pos_velo = new_detections[i]
                    else:
                        emb, box, logit, score, global_pts, pos_velo = default_embedding, default_box, default_logit, default_score, default_global_pts, default_pos_velo
                    all_embeddings.append(emb)
                    all_boxes.append(box)
                    all_logits.append(logit)
                    all_scores.append(score)
                    all_global_pts.append(global_pts)
                    all_pos_velo.append(pos_velo)
                ...
                all_global_pts = torch.stack(all_global_pts)
                all_pos_velo = torch.stack(all_pos_velo)
                ...
                det_dict = {'pred_logits': detector.pred_logits.unsqueeze(0).expand(num_frame,-1,-1)[:,None],
                        'pred_boxes': detector.pred_boxes.unsqueeze(0).expand(num_frame, -1,-1)[:,None]}
    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            img_single_v = img[frame_idx][:,0,:,:,:].unsqueeze(1)   #[1,10,3,928,1600]  [batch, img_num, rgb, height, width]
            img_single_i = img[frame_idx][:,6,:,:,:].unsqueeze(1)
            img_metas_single_v = deepcopy(img_metas)
            img_metas_single_i = deepcopy(img_metas)


            img_metas_single_v[0]['lidar2ego'][frame_idx] = [img_metas_single_v[0]['lidar2ego'][frame_idx][0]]
            img_metas_single_v[0]['l2g_r_mat'] = [img_metas_single_v[0]['l2g_r_mat'].data[frame_idx][0]]
            img_metas_single_v[0]['l2g_t'] = [img_metas_single_v[0]['l2g_t'].data[frame_idx][0]]
                
            if 'uni_rot_aug' in img_metas_single_v[0]:
                img_metas_single_v[0]['uni_rot_aug'] = [img_metas_single_v[0]['uni_rot_aug'][frame_idx][0]]
                    
                
            img_metas_single_v[0]['pad_shape'] = [img_metas_single_v[0]['pad_shape'][frame_idx][0]]
            img_metas_single_v[0]['img_shape'] = [img_metas_single_v[0]['img_shape'][frame_idx][0]]
            img_metas_single_v[0]['timestamp'] = img_metas_single_v[0]['timestamp'][frame_idx]
            img_metas_single_v[0]['lidar2img'] = [img_metas_single_v[0]['lidar2img'][frame_idx][0]]
            img_metas_single_v[0]['lidar2global'] = [img_metas_single_v[0]['lidar2global'][frame_idx][0]]
            img_metas_single_v[0]['box_type_3d'] = img_metas_single_v[0]['box_type_3d'][frame_idx]
            img_metas_single_v[0]['gt_bboxes_3d'] = img_metas_single_v[0]['gt_bboxes_3d']._data[frame_idx]
            img_metas_single_v[0]['gt_labels_3d'] = img_metas_single_v[0]['gt_labels_3d']._data[frame_idx]
            img_metas_single_v[0]['filename'] = [img_metas_single_v[0]['filename'][frame_idx][0]]
            img_metas_single_v[0]['ori_shape'] = img_metas_single_v[0]['ori_shape'][frame_idx]
            img_metas_single_v[0]['cam_intrinsic'] = [img_metas_single_v[0]['cam_intrinsic'][frame_idx][0]]
            ...
            img_metas_single_i[0]['ori_shape'] = img_metas_single_i[0]['ori_shape'][frame_idx]
            img_metas_single_i[0]['cam_intrinsic'] = [img_metas_single_i[0]['cam_intrinsic'][frame_idx][6]]

            
            # Modification: Do late fusion here
            
            detector_v = self.forward_single(img_single_v.squeeze(1), detector_v, img_metas_single_v, frame_idx=frame_idx)
            detector_i = self.forward_single(img_single_i.squeeze(1), detector_i, img_metas_single_i, frame_idx=frame_idx, tracklist=tracklist, infra_label=True)
            ...
            threshold = 0.5
            matched_v, matched_i = torch.where(sim_matrix > threshold)
            #print(matched_v, matched_i)
            
            merged_v, merged_i = [], []
            new_detections = {}

            diff = detector_v.pred_boxes[matched_v, :2] - detector_i.pred_boxes[matched_i, :2]
            dist = torch.norm(diff, dim=-1)
            valid_mask = dist <= 1
            matched_v = matched_v[valid_mask]
            matched_i = matched_i[valid_mask]
            ...
            #plt_area = 255 * np.ones((h, w, 3), dtype=np.uint8)
            #fig, ax = plt.subplots()
            ...
            #for idx in range(900):
                #if detector_v.scores[idx] < 0.25 and idx not in merged_v:
                    #continue
                #if detector_v.scores[idx] < 0.25 and idx in merged_v:
                    #idx_i = merged_i[merged_v.index(idx)]
                    #if detector_i.scores[idx_i] < 0.25:
                        #continue
                    #else:
                        #pass
                #width, height = detector_v.pred_boxes[idx][2], detector_v.pred_boxes[idx][3]
                #xmin, ymin = detector_v.pred_boxes[idx][0] - width/2, detector_v.pred_boxes[idx][1] - height/2
                #xmin = float(xmin.cpu()) if torch.is_tensor(xmin) else xmin
                #ymin = float(ymin.cpu()) if torch.is_tensor(ymin) else ymin
                #width = float(width.cpu()) if torch.is_tensor(width) else width
                #height = float(height.cpu()) if torch.is_tensor(height) else height
                #if idx in merged_v:
            ...
            #for idx in range(900):
                #if detector_i.scores[idx] < 0.25 and idx not in merged_i:
                    #continue
                #if detector_i.scores[idx] < 0.25 and idx in merged_i:
                    #idx_v = merged_v[merged_i.index(idx)]
                    #if detector_v.scores[idx_v] < 0.25:
                        #continue
                    #else:
                        #pass
                #width, height = detector_i.pred_boxes[idx][2], detector_i.pred_boxes[idx][3]
                #xmin, ymin = detector_i.pred_boxes[idx][0] - width/2, detector_i.pred_boxes[idx][1] - height/2
            ...
            for i in unmatched_i:
                ...
                new_detections[i+900] = (detector_i.output_embedding[i], detector_i.pred_boxes[i], detector_i.pred_logits[i], detector_i.scores[i], detector_i.global_pts[i], detector_i.pos_velo[i])
            ...
            #num_high = (detector.scores > 0.4).sum().item()
            #print(num_high)
            #detector = deepcopy(detector_v)
    ...
    def generate_empty_detector(self,label=False):
        ...
        detector.global_pts = torch.zeros(
            (num_det, 3), dtype=torch.float, device=device)
    ...
    def generate_empty_tracker(self):
        ...
        tracker.global_pts = torch.zeros(
            (num_track, 3), dtype=torch.float, device=device)
    ...
    def _copy_tracks(self, tgt_tracker):
        ...
        tracker.global_pts = tgt_tracker.ref_pts.clone()
    ...
    def _delete_tracks(self, tracker, dead_track):
        ...
        tracker.ref_pts[dead_track] = torch.zeros(
            (num_track, 3), dtype=torch.float, device=device)
        tracker.global_pts[dead_track] = torch.zeros(
            (num_track, 3), dtype=torch.float, device=device)
    ...
    def tracklet2results(self, tracker, img_metas):
        ...
        out_dict. keys:
            ...
            - tracking_score
        '''
        if self.tracker_cfg.get('eval_det_only', True):
            threshold = 0
            mask = tracker.scores > threshold
