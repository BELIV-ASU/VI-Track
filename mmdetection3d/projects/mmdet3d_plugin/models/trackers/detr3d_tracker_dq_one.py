...
class DETR3DTrackerDQ(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.proj_net = nn.Sequential(
            nn.Linear(275, tracker_cfg.embed_dims),
            #nn.LayerNorm(tracker_cfg.embed_dims),
            nn.ReLU(),
            nn.Linear(tracker_cfg.embed_dims, 3*tracker_cfg.embed_dims),
            #nn.LayerNorm(3*tracker_cfg.embed_dims),
            nn.ReLU(),
            nn.Linear(3*tracker_cfg.embed_dims, 3*tracker_cfg.embed_dims)
        )
        self.value_head = nn.Linear(3*tracker_cfg.embed_dims, tracker_cfg.embed_dims*500)
        self.pixel_num = 500
        #self.value_head = nn.Linear(3*tracker_cfg.embed_dims, self.pixel_num)
        #self.value_emb = nn.Linear(1,tracker_cfg.embed_dims) 

        self.loc_head = nn.Linear(3*tracker_cfg.embed_dims, 2*self.pixel_num)
        self.mask_head = nn.Sequential(nn.LayerNorm(3*tracker_cfg.embed_dims), nn.Linear(3*tracker_cfg.embed_dims, 1), nn.Sigmoid())
        
        self.injection_proj = nn.Conv2d(3, 256, kernel_size=1)
        self.fusion_proj = nn.Linear(3*tracker_cfg.embed_dims, tracker_cfg.embed_dims)
    ...
    def embedding_interaction(self, detector, tracklist=[], delay=None):
        ...
        if len(tracklist) >= 2:
            ...
            track_past_list = [tracklist[-2].query_track[valid_mask], tracklist[-1].query_track[valid_mask]]
            track_past_embedding = torch.stack(track_past_list, dim=0).reshape(-1, 256)
            ## Create the temporal embeddings
            time_slot = torch.tensor([[0.05], [0.1]]) + torch.randn(2, 1)*0.1
    ...
    def forward_detector(self, img, img_metas, detector, tracklist=[], infra_label=False):
        ...
        if len(tracklist) > 0:
            ...
            img_flat = img.view(B * T, 3, 8*H, 8*W)
            img_downsampled = F.interpolate(img_flat, size=img_feats[0].shape[-2:], mode='bilinear', align_corners=False)
            ...
            # input [track_query, global_pts, camera_intrinsic]-> [picture_index, value, pixel_loc]
            K_flat = K.view(B*T, -1).unsqueeze(1)
            pix_num = self.pixel_num

            valid_track = tracklist[-1].valid_track
            his_query = tracklist[-1].query_track[valid_track]
            his_pos_global = tracklist[-1].global_pts[valid_track]
            his_query_exp = his_query.unsqueeze(1).repeat(1, B*T, 1).permute(1,0,2)
            his_pos_exp = his_pos_global.unsqueeze(1).repeat(1, B*T, 1).permute(1,0,2)
            K_flat_exp = K_flat.repeat(1, his_pos_exp.shape[1], 1)
            
            inputs = torch.cat([K_flat_exp, his_query_exp, his_pos_exp], dim=-1)  # [B*T, num, 16+256+3]

            # Step 2: Projection network output
            
            inputs_feat = self.proj_net(inputs)  # [B*T, num, dim * 3]
            value_output = self.value_head(inputs_feat)
            loc = self.loc_head(inputs_feat)
            mask_label = self.mask_head(inputs_feat)

            #value_expand = value_output.unsqueeze(-1)
            #value_emb = self.value_emb(value_expand)
            #value_output = value_emb.reshape(B*T, inputs_feat.shape[1], -1)
            
            # Step 3: Extract binary mask from last dim
            outputs_mask = mask_label              # [B*T, num, 1]
            outputs_binary_mask = (outputs_mask > 0.2).float()           # [B*T, num, 1]
            outputs_mask_exp = outputs_binary_mask.unsqueeze(2).unsqueeze(3)  # [B*T, num, 1, 1]

            # Step 5: Reshape outputs
            outputs = value_output.reshape(B*T, his_query_exp.shape[1], his_query_exp.shape[2], pix_num, 1)  # [B*T, num, dim, pix_num, 1]
            
            # Step 6: Apply binary mask
            outputs_masked = outputs * outputs_mask_exp  # same shape
            
            loc_reshape = loc.reshape(B*T, his_query_exp.shape[1], 1, pix_num, 2)
            # Step 8: Split into values and coordinates
            values = outputs_masked.squeeze(-1)  # [B*T, num, dim, pix_num]
            loc_reshape = ((loc_reshape + 1) / 2)  # [0, 1]
            loc_reshape[..., 0] *= (W - 1)
            loc_reshape[..., 1] *= (H - 1)
            
            u = loc_reshape[..., 0].round().long().clamp(0, W - 1)
            v = loc_reshape[..., 1].round().long().clamp(0, H - 1)
            
            # Step 9: Convert (u, v) to flat pixel index
            flat_idx = v * W + u  # [B*T, dim, pix_num]

            values_flat = values.permute(0, 2, 1, 3).reshape(B*T, his_query_exp.shape[2], -1)      # [B*T, C, N]
            flat_idx_flat = flat_idx.permute(0, 2, 1, 3).reshape(B*T, 1, -1)  # [B*T, C, N]
            
            image_accum = torch.zeros(B*T, C, H*W, device=values.device)  # for accumulated value
            image_count = torch.zeros(B*T, 1, H*W, device=values.device)    # for counting hits
            image_accum.scatter_add_(2, flat_idx_flat.expand(-1, C, -1), values_flat)
            image_count.scatter_add_(2, flat_idx_flat, torch.ones_like(flat_idx_flat, dtype=values.dtype))
            img_injected = image_accum / (image_count + 1e-6)
            

            # Step 12: Reshape and add to original img_feats
            img_injected = img_injected.view(B, T, C, H, W)

            mask = (img_injected < 0).float()
            img_sum = F.avg_pool3d(img_injected * mask, kernel_size=5, stride=1, padding=2)
            mask_sum = F.avg_pool3d(mask, kernel_size=5, stride=1, padding=2)
            mask_sum[mask_sum == 0] = 1e-6
            img_injected_smoothed = img_sum / mask_sum


            non_zero_mask = img_injected.mean(dim=2)<0
            sampled_feats = img_downsampled * non_zero_mask.permute(1,0,2,3)
            sampled_feats_proj = self.injection_proj(sampled_feats).view(B, T, 256, H, W)

            mask = (sampled_feats_proj != 0).float()
            img_sum = F.avg_pool3d(sampled_feats_proj * mask, kernel_size=5, stride=1, padding=2)
            mask_sum = F.avg_pool3d(mask, kernel_size=5, stride=1, padding=2)
            mask_sum[mask_sum == 0] = 1e-6
            img_sampled_smoothed = img_sum / mask_sum
            
            
            #img_feats[0] = img_feats[0] + img_injected_smoothed + img_sampled_smoothed
            concat_feats = torch.cat([img_feats[0], img_injected, sampled_feats_proj], dim=2)
            #concat_feats = torch.cat([img_feats[0], img_injected_smoothed, img_sampled_smoothed], dim=2)
            concat_feats_reshape = concat_feats.permute(0, 1, 3, 4, 2).contiguous().view(-1, 3*C)
            fuse_feats = self.fusion_proj(concat_feats_reshape)
            img_feats[0] = fuse_feats.view(B, T, H, W, -1).permute(0, 1, 4, 2, 3).contiguous()
    ...
    def forward_train(self,
        ...
        for batch_idx in range(batch_num):
            ...
            for frame_idx in range(num_frame):
                ...
                img_single_v = img[batch_idx][frame_idx][None][:,0,:,:,:].unsqueeze(1)   #[1,10,3,928,1600]  [batch, img_num, rgb, height, width]
                
                img_single_i = img[batch_idx][frame_idx][None][:,7,:,:,:].unsqueeze(1)
                ...
                img_metas_single_i[0]['lidar2ego'][frame_idx] = [img_metas_single_i[0]['lidar2ego'][frame_idx][7]]
                img_metas_single_i[0]['l2g_r_mat'] = [img_metas_single_i[0]['l2g_r_mat'].data[frame_idx][7]]
                img_metas_single_i[0]['l2g_t'] = [img_metas_single_i[0]['l2g_t'].data[frame_idx][7]]
                if 'uni_rot_aug' in img_metas_single_i[0]:
                    img_metas_single_i[0]['uni_rot_aug'] = [img_metas_single_i[0]['uni_rot_aug'][frame_idx][7]]
                img_metas_single_i[0]['pad_shape'] = [img_metas_single_i[0]['pad_shape'][frame_idx][7]]
                img_metas_single_i[0]['img_shape'] = [img_metas_single_i[0]['img_shape'][frame_idx][7]]
                img_metas_single_i[0]['timestamp'] = [img_metas_single_i[0]['timestamp'][frame_idx]]
                img_metas_single_i[0]['lidar2img'] = [img_metas_single_i[0]['lidar2img'][frame_idx][7]]
                img_metas_single_i[0]['lidar2global'] = [img_metas_single_i[0]['lidar2global'][frame_idx][7]]
                ...
                img_metas_single_i[0]['filename'] = [img_metas_single_i[0]['filename'][frame_idx][7]]
                
                img_metas_single_i[0]['ori_shape'] = img_metas_single_i[0]['ori_shape'][frame_idx]
                img_metas_single_i[0]['cam_intrinsic'] = [img_metas_single_i[0]['cam_intrinsic'][frame_idx][7]]
                ...
                combined_features_v = torch.cat([detector_v.pos_velo, detector_v.pred_logits, detector_v.pred_boxes], dim=-1)
                
                expanded_features_v = self.mlp(combined_features_v)
                embedding_v_proj = self.emb_proj(detector_v.output_embedding)
                fused_embedding_v = expanded_features_v + embedding_v_proj
                #fused_embedding_v = expanded_features_v + detector_v.output_embedding
                                           
                combined_features_i = torch.cat([detector_i.pos_velo, detector_i.pred_logits, detector_i.pred_boxes], dim=-1)
                
                expanded_features_i = self.mlp(combined_features_i)
                embedding_i_proj = self.emb_proj(detector_i.output_embedding)
                fused_embedding_i = expanded_features_i + embedding_i_proj
                ...
                for idx_v, idx_i in zip(matched_v.tolist(), matched_i.tolist()):
                    ...
                    if detector_v.scores[idx_v] >= detector_i.scores[idx_i]:
                        new_detections[idx_v] = (geo_v[:256], geo_v[256:266], geo_v[266:276], geo_v[276], geo_v[277:280], geo_v[280:284])
                    else:
                        new_detections[idx_v] = (geo_i[:256], geo_i[256:266], geo_i[266:276], geo_i[276], geo_i[277:280], geo_i[280:284])
