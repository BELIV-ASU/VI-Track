...
class DETR3DTrackerDQ(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.fusion_mlp = nn.Sequential(nn.Linear(562, 512),
                                        nn.LayerNorm(512),
                                        nn.ReLU(),
                                        nn.Linear(512, 281),
                                        nn.Sigmoid())
        ...
        if not self.train_det_only:
            ...
            self.refined_query_inter = QueryInteractionX(
                                    tracker_cfg.embed_dims,
                                    ffn_channel,
                                    **self.tracker_cfg.query_trans)
    ...
    def tracklet_asso_test(self, tracker, detector, tracklist=[], time_delta=None, img_metas=None):
        ...
        #print('valid detector', detector[valid_det].output_embedding.shape)
        valid_track = tracker.valid_track.clone()
        #print('valide track', tracker[valid_track].query_track.shape)
    ...
    def sort_and_filter(self, detector, img_metas):
        ...
        l2e_i_mat = torch.from_numpy(img_metas[0]['lidar2ego'][0][6]).to(pred_cls.device)
        ego_pts = torch.cat([detector.ref_pts, 
                                torch.ones(len(detector),1).to(detector.ref_pts.device)],dim=1)
        ego_pts[:900] = ego_pts[:900] @ l2e_mat.T
        ego_pts[900:] = ego_pts[900:] @ l2e_i_mat.T
    ...
    def forward_detector(self, img, img_metas, detector, infra_label=False):
    ...
    def forward_single(self, img, detector, img_metas, depth_map=None, frame_idx=0, infra_label=False):
        pred_dict, detector = self.forward_detector(img, img_metas, detector, infra_label)
            ...
            for frame_idx in range(num_frame):
                ...
                if self.tracker_cfg.get('train_track_only', False):
                    with torch.no_grad():
                        ...
                        detector_i, loss_single_i, info_single_i = self.forward_single(img_single_i,detector_i, img_metas_single_i,depth_map,frame_idx,infra_label=True)
                else:
                    # calculate detection loss and matched pairs for each frame
                    with torch.no_grad():
                        detector_v, loss_single_v, info_single_v = self.forward_single(img_single_v,detector_v, img_metas_single_v,depth_map,frame_idx,infra_label=False)
                ...
                for idx_v, idx_i in zip(matched_v.tolist(), matched_i.tolist()):
                    ...
                    geo_v = torch.cat((detector_v.output_embedding[idx_v],detector_v.pred_boxes[idx_v],detector_v.pred_logits[idx_v],detector_v.scores[idx_v].unsqueeze(0), detector_v.pos_velo[idx_v]),dim=0)
                    geo_i = torch.cat((detector_i.output_embedding[idx_i],detector_i.pred_boxes[idx_i],detector_i.pred_logits[idx_i],detector_i.scores[idx_i].unsqueeze(0), detector_i.pos_velo[idx_i]),dim=0)
                    ...
                    new_detections[idx_v] = (geo_fusion[:256], geo_fusion[256:266], geo_fusion[266:276], geo_fusion[276], geo_fusion[277:281])
                ...
                for i in unmatched_v:
                    new_detections[i] = (detector_v.output_embedding[i], detector_v.pred_boxes[i], detector_v.pred_logits[i], detector_v.scores[i], detector_v.pos_velo[i])
                ...
                for i in unmatched_i:
                    if i in info_single_i['pos_inds']:
                        ...
                        new_detections[i+900] = (detector_i.output_embedding[i], detector_i.pred_boxes[i], detector_i.pred_logits[i], detector_i.scores[i], detector_i.pos_velo[i])
                ...
                for i in range(N):
                    if i in new_detections:
                        emb, box, logit, score, pos_velo = new_detections[i]
                    else:
                        emb, box, logit, score, pos_velo = default_embedding, default_box, default_logit, default_score, default_pos_velo
    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            detector_i = self.forward_single(img_single_i, detector_i, img_metas_single_i, frame_idx=frame_idx, infra_label=True)
            ...
            if self.tracker_cfg.get('eval_det_only', False):
                ...
                tracker = self.tracklet_asso_test(tracker, detector, tracklist, time_delta,img_metas)
