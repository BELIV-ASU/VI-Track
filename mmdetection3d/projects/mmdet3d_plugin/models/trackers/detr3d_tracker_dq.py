    ...
    def forward_test(self, 
        ...
        for frame_idx in range(num_frame):
            ...
            for idx in range(900):
                if detector_v.scores[idx] < 0.2 and idx not in merged_v:
                    continue
                if detector_v.scores[idx] < 0.2 and idx in merged_v:
                    idx_i = merged_i[merged_v.index(idx)]
                    if detector_i.scores[idx_i] < 0.2:
                        continue
                    else:
                        pass
            ...
            for idx in range(900):
                if detector_i.scores[idx] < 0.2 and idx not in merged_i:
                    continue
                if detector_i.scores[idx] < 0.2 and idx in merged_i:
                    idx_v = merged_v[merged_i.index(idx)]
                    if detector_v.scores[idx_v] < 0.2:
                        continue
                    else:
                        pass
            ...
            for v_idx, i_idx in zip(merged_v, merged_i):

                # if both detections are low-score, skip
                if detector_v.scores[v_idx] < 0.25 and detector_i.scores[i_idx] < 0.25:
                    continue

                # vehicle box
                cx_v, cy_v, w_v, h_v = detector_v.pred_boxes[v_idx][0], detector_v.pred_boxes[v_idx][1], detector_v.pred_boxes[v_idx][2], detector_v.pred_boxes[v_idx][3] 
                # infra box
                cx_i, cy_i, w_i, h_i = detector_i.pred_boxes[i_idx][0], detector_i.pred_boxes[i_idx][1], detector_i.pred_boxes[i_idx][2], detector_i.pred_boxes[i_idx][3]

                # convert tensors -> floats
                to_f = lambda x: float(x.cpu()) if torch.is_tensor(x) else float(x)
                cx_v, cy_v, w_v, h_v = map(to_f, [cx_v, cy_v, w_v, h_v])
                cx_i, cy_i, w_i, h_i = map(to_f, [cx_i, cy_i, w_i, h_i])

                # score weighted fusion (better than plain averaging)
                sv = 0.5 #float(detector_v.scores[v_idx])
                si = 0.5 #float(detector_i.scores[i_idx])
                wsum = sv + si + 1e-6

                cx_f = (cx_v * sv + cx_i * si) / wsum
                cy_f = (cy_v * sv + cy_i * si) / wsum
                w_f  = (w_v  * sv + w_i  * si) / wsum
                h_f  = (h_v  * sv + h_i  * si) / wsum

                # convert center → top-left
                xmin_f = cx_f - w_f / 2
                ymin_f = cy_f - h_f / 2

                # ---- Draw fused bounding box ----
                fused_rect = patches.Rectangle(
                    (xmin_f, ymin_f), w_f, h_f,
                    linewidth=2, edgecolor='k', facecolor='none'
                )
                ax.add_patch(fused_rect)
