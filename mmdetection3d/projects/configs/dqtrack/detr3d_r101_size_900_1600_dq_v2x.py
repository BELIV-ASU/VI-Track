...
model = dict(
    ...
    tracker_cfg=dict(
        ...
        num_track=300,
        num_query=1800,
        num_cams=10, #6, change it in /home/jmeng18/DQTrack-main/mmdetection3d/projects/mmdet3d_plugin/models/utils/detr3d_transformer.py Detr3DCrossAttenRaw
...
test_pipeline_post = [
    dict(type='FormatBundle3DTrack'),
    dict(type='CollectUnified3D', keys=['img', 'timestamp', 'gt_bboxes_3d', 'gt_labels_3d'])
]
...
data = dict(
    ...
    train=dict(
        ...
        use_valid_flag=True,
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, data_root=data_root, pipeline=test_pipeline, pipeline_post=test_pipeline_post, 
    ...
    test=dict(type=dataset_type, data_root=data_root, pipeline=test_pipeline, pipeline_post=test_pipeline_post, 
             classes=class_names, track_classes=track_names, modality=input_modality, 
             ann_file=info_root + "track_cat_10_infos_val.pkl",
             num_frames_per_sample=1)) # please change to your own info file
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            #'img_backbone_i':dict(lr_mult=0.1),
        }),
    weight_decay=0.01)
