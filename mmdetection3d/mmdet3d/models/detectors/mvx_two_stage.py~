...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if pts_bbox_head:
            ...
            # Modification
            head_cfg_v = copy.deepcopy(pts_bbox_head)
            head_cfg_i = copy.deepcopy(pts_bbox_head)
            head_cfg_v['transformer']['decoder']['transformerlayers']['attn_cfgs'][1]['num_cams'] = 6
            head_cfg_i['transformer']['decoder']['transformerlayers']['attn_cfgs'][1]['num_cams'] = 4
            pts_bbox_head['transformer']['decoder']['transformerlayers']['attn_cfgs'][1]['num_cams'] = 6
            self.pts_bbox_head = builder.build_head(head_cfg_v)
            pts_bbox_head['transformer']['decoder']['transformerlayers']['attn_cfgs'][1]['num_cams'] = 4
            self.pts_bbox_head_i = builder.build_head(head_cfg_i)

        if img_backbone:
            self.img_backbone = builder.build_backbone(img_backbone)
            self.img_backbone_i = builder.build_backbone(img_backbone)
            
        if img_neck is not None:
            self.img_neck = builder.build_neck(img_neck)
            self.img_neck_i = builder.build_neck(img_neck)
